...
const ValidateIntro: React.FC<ValidateIntroProps> = ({
  recoveryPhrase,
  onGoBack,
  onComplete,
}) => {
  ...
  const [inputWords, setInputWords] = useState<string[]>(
    Array(recoveryPhrase.length).fill(""),
  );
  const [errors, setErrors] = useState<boolean[]>(
    Array(recoveryPhrase.length).fill(false),
  );
  ...
  const handleContinueClick = () => {
    const newErrors = inputWords.map(
      (word, idx) => word.trim() !== recoveryPhrase[idx],
    );
    setErrors(newErrors);
  ...
  return (
    <div
      className="container min-h-screen py-5 relative overflow-y-scroll"
      onPaste={handlePaste}
    >
      <Header
        title={
      ...
      <div className="grid grid-cols-2 justify-start items-start gap-5 my-5">
        {recoveryPhrase.map((_, index) => (
          <div
            key={index}
            className={`bg-primary/20 rounded-full justify-start items-center inline-flex`}
          >
            <div
              className={`justify-center items-center flex w-8 h-10 rounded-s-full pl-2 pr-1 ${errors[index] ? "bg-error" : ""} ${isProcessing ? "bg-primary" : ""}`}
            >
              <span className="text-center text-white text-lg font-normal select-none">
                {index + 1}
              </span>
            </div>
