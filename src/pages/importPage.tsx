    ...
    >
      ...
      <div className="grid grid-cols-2 gap-5 my-5">
        {inputWords.map((word, index) => (
          <div
            key={index}
            className="bg-primary/20 rounded-full flex items-center"
          >
            <div
              className={`flex justify-center items-center w-8 h-10 rounded-s-full pl-2 pr-1 ${isProcessing ? "bg-primary" : ""}`}
            >
              <span className="text-white text-lg font-normal select-none">
                {index + 1}
              </span>
            </div>
