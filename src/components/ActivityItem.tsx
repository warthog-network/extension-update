...
const ActivityItem: React.FC<ActivityItemProps> = ({
  ...
  <>
    {activities.map((item, index) => (
      <div
        key={index}
        className="flex-col gap-[15px] inline-flex w-full my-4"
      ...
      >
        ...
        <div className="flex justify-between items-center gap-3 w-full">
          ...
          <div>
            <h5 className="text-right text-white text-xl font-medium">
              {item.amount}
            </h5>
            <p className="text-right text-white/50 text-lg font-normal">
              {item.usdAmount}
            </p>
          </div>
        </div>
